...
func (k Keeper) checkMisbehaviour(ctx sdk.Context, evidence exported.Evidence) error {
	...
	// 	if !ok {
	// 		return comdexCrustErrors.Wrap(types.ErrInvalidClientType(k.codespace), "consensus type is not Tendermint")
	// 	}
...
func (k Keeper) freeze(ctx sdk.Context, clientState types.State) (types.State, error) {
	if clientState.Frozen {
		return types.State{}, comdexCrustErrors.Wrap(types.ErrClientFrozen(k.codespace, clientState.ID()), "already frozen")
	}

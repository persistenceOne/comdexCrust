...
// EmptyTags : New empty tags
func EmptyTags() Tags {
	return make(Tags, 0)
}

// AppendTag : Append a single tag
func (t Tags) AppendTag(k string, v []byte) Tags {
	return append(t, MakeTag(k, v))
}

// AppendTags : Append two lists of tags
func (t Tags) AppendTags(tags Tags) Tags {
	return append(t, tags...)
}

// ToKVPairs : Turn tags into KVPair list
func (t Tags) ToKVPairs() []cmn.KVPair {
	return []cmn.KVPair(t)
}

// NewTags : New variadic tags, must be k string, v []byte repeating
...
// MakeTag Make a tag from a key and a value

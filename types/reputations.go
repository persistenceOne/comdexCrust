...
func NewTraderFeedback(buyerAddress cTypes.AccAddress, sellerAddress cTypes.AccAddress, pegHash PegHash, rating int64) TraderFeedback {
...
func (traderFeedback TraderFeedback) GenerateNegotiationID() []byte {
	return append(append(traderFeedback.BuyerAddress.Bytes(), traderFeedback.SellerAddress.Bytes()...), traderFeedback.PegHash.Bytes()...)
}
...
func (baseAccountReputation *BaseAccountReputation) AddTraderFeedback(traderFeedback TraderFeedback) cTypes.Error {
	...
	return ErrFeedbackCannotRegister(DefaultCodeSpace)
}

...
// QueryWithData information about the connected node with a data payload
...
// EnsureBroadcastTxWithResponse : EnsureBroadcastTx With Response
func (ctx CLIContext) EnsureBroadcastTxWithResponse(txBytes []byte) ([]byte, error) {
	if ctx.Async {
		return ctx.ensureBroadcastTxAsyncWithResponse(txBytes)
	}
	return ctx.ensureBroadcastTxWithResponse(txBytes)
}
...
func (ctx CLIContext) ensureBroadcastTxAsyncWithResponse(txBytes []byte) ([]byte, error) {
	res, err := ctx.BroadcastTxAsync(txBytes)
	if err != nil {
		return nil, err
	}

...
// SendFiatHandlerFunction : handles send fiat rest message
func SendFiatHandlerFunction(cliCtx context.CLIContext, cdc *wire.Codec, kb keys.Keybase, kafka bool, kafkaState rest.KafkaState) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		var msg fiatFactory.SendFiatBody
		...
		buildmsg := fiatFactory.BuildSendFiatMsg(from, ownerAddress, to, assetPegHashHex, sdk.FiatPegWallet{fiatPeg})
		
		if kafka == true {
			ticketID := rest.TicketIDGenerator("FFSF")

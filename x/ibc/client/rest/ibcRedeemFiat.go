...
// RedeemFiatHandlerFunction : IBC Redeem Fiat
func RedeemFiatHandlerFunction(cdc *wire.Codec, kb keys.Keybase, cliCtx context.CLIContext, kafka bool, kafkaState rest.KafkaState) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		
		var msg ibc.RedeemFiatBody
		...
		fiatPeg := sdk.BaseFiatPeg{
			PegHash:        pegHashHex,
			RedeemedAmount: msg.RedeemAmount,
		}
		
		redeemMsg := ibc.BuildRedeemFiatMsg(from, to, msg.RedeemAmount, sdk.FiatPegWallet{fiatPeg}, msg.SourceChainID, msg.DestinationChainID)
		
		if kafka == true {
			ticketID := rest.TicketIDGenerator("IBCRF")
			jsonResponse := rest.SendToKafka(rest.NewKafkaMsgFromRest(redeemMsg, ticketID, txCtx, cliCtx, msg.Password), kafkaState, cdc)
			w.WriteHeader(http.StatusAccepted)
			w.Write(jsonResponse)
			
		} else {
			output, err := utils.SendTxWithResponse(txCtx, cliCtx, []sdk.Msg{redeemMsg}, msg.Password)
			if err != nil {
				utils.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
				return
			}

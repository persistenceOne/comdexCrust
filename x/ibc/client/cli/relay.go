...
func (c relayCommander) refine(bz []byte, sequence int64, passphrase string) []byte {
	var msg sdk.Msg
	if err := c.cdc.UnmarshalBinary(bz, &msg); err != nil {
		panic(err)
	}
	switch msg := msg.(type) {
	...
	case ibc.MsgRedeemFiats:
		...
		return res
	case ibc.MsgBuyerExecuteOrders:
		relayMsg := ibc.NewMsgRelayBuyerExecuteOrders(msg.BuyerExecuteOrders, c.address, sequence)
		txCtx := context2.NewTxContextFromCLI().WithSequence(sequence).WithCodec(c.cdc)
		cliCtx := context.NewCLIContext()
		res, err := txCtx.BuildAndSign(cliCtx.FromAddressName, passphrase, []sdk.Msg{relayMsg})
		if err != nil {
		...
		return res
	case ibc.MsgSellerExecuteOrders:
		relayMsg := ibc.NewMsgRelaySellerExecuteOrders(msg.SellerExecuteOrders, c.address, sequence)
		txCtx := context2.NewTxContextFromCLI().WithSequence(sequence).WithCodec(c.cdc)
		cliCtx := context.NewCLIContext()
		res, err := txCtx.BuildAndSign(cliCtx.FromAddressName, passphrase, []sdk.Msg{relayMsg})
		if err != nil {
		...
		return res
	case ibc.IBCTransferMsg:
		relayMsg := ibc.IBCReceiveMsg{
			IBCPacket: msg.IBCPacket,
			Relayer:   c.address,
			Sequence:  sequence,
		}

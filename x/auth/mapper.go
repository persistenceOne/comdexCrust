...
// NewAccountWithAddress Implaements sdk.AccountMapper.
...
// GetAccount : Implements sdk.AccountMapper.
...
// SetAccount : Implements sdk.AccountMapper.
...
// IterateAccounts : Implements sdk.AccountMapper.
...
// GetNextAssetPegHash : Returns and increments the assetPeg counter
func (am AccountMapper) GetNextAssetPegHash(ctx sdk.Context) int {
	var assetNumber int
	store := ctx.KVStore(am.key)
	bz := store.Get(assetPegHashKey)
	if bz == nil {
		assetNumber = 0
	} else {
		err := am.cdc.UnmarshalBinary(bz, &assetNumber)
		if err != nil {
			panic(err)
		}
	}
...
func (am AccountMapper) GetNextFiatPegHash(ctx sdk.Context) int {
	var fiatNumber int
	store := ctx.KVStore(am.key)
	bz := store.Get(fiatPegHashKey)
	if bz == nil {
		fiatNumber = 0
	} else {
		err := am.cdc.UnmarshalBinary(bz, &fiatNumber)
		if err != nil {
			panic(err)
		}
	}

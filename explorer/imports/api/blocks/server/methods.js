...
Meteor.methods({
    ...
    'blocks.getCurrentHeight': function() {
        this.unblock();
        console.log("BlockCons----",Blockscon.find({},{sort:{height:-1},limit:1}).fetch())
    ...
    'blocks.blocksUpdate': function() {
        ...
        console.log("updateBlockes list",curr);
            ...
            for (let height = curr+1 ; height <= until ; height++) {
                ...
                try{
                    ...
                    if (response.statusCode == 200){
                        ...
                        // get the proposer address from dump_consensus_state   
                        let url = RPC+"/dump_consensus_state";
                        try{
                            let responseConsensus = HTTP.get(url)
                            let consensus = JSON.parse(responseConsensus.content)
                            consensus = consensus.result
                            blockData.proposerAddress = consensus.round_state.validators.proposer.address;
                        ...
                        if (block.block.data.txs && block.block.data.txs.length > 0){
                            for (t in block.block.data.txs){
                                console.log( sha256(Buffer.from(block.block.data.txs[t], 'base64')))
                        ...
                        if (validators.result){
                            for (v in validators.result.validators){
                                ...
                                if (!valExist){
                                    ...
                                    for (val in validatorSet){
                                        console.log(validatorSet[val],"data",valExist)
                                        if (validatorSet[val].pub_key == valExist.consensus_pubkey){
                                            validator.jailed = validatorSet[val].jailed;
                                            validator.status = validatorSet[val].status;
                                            validator.tokens = validatorSet[val].tokens;
                                            validator.delegator_shares =validatorSet[val].delegator_shares;
                                            ...
                                            // calculate self delegation percentage every 5 blocks
                                            if (height % 5 == 0){
                                                try{

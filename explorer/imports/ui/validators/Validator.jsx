...
export default class Validator extends Component{
    ...
    getUserDelegations() {
        if (this.state.user && this.props.validator && this.props.validator.address) {
            Meteor.call('accounts.getDelegation', this.state.user, this.props.validator.address, (err, res) => {
                if (res && res.shares > 0) {
    ...
    isSameValidator(prevProps) {
        ...
        if (this.props.validator == null || prevProps.validator == null)
            return false
        return this.props.validator.operator_address === prevProps.validator.operator_address;
    }
        ...
        if (this.props.loading){
            ...
            if (this.props.validatorExist){
                ...
                return <Row className="validator-details">
                    <Helmet>
                        <title>{ moniker } - Commit Validator </title>
                    ...
                    <Card>
                          ...
                          {/* <Link className="float-right" to={"/validator/"+this.props.validator.address+"/missed/blocks"}><T>common.more</T>...</Link>
                          */}
                    ...
                    <Col md={8}>
                        ...
                        <CardBody>
                            <Row>
                                ...
                                <Col sm={8} className="value">{this.props.validator.commission?numbro(this.props.validator.commission*100).format('0.00')+"%":''} <small className="text-secondary">({this.state.updateTime})</small></Col>
                                <Col sm={4} className="label"><T>validators.maxRate</T></Col>
                                <Col sm={8} className="value">{this.props.validator.commission?numbro(this.props.validator.commission_max*100).format('0.00')+"%":''}</Col>
                                <Col sm={4} className="label"><T>validators.maxChangeRate</T></Col>
                                <Col sm={8} className="value">{this.props.validator.commission?numbro(this.props.validator.commission_change_rate*100).format('0.00')+"%":''}</Col>
                            </Row>
                          </CardBody>
                      </Card>
                        <Card>
                    ...
                    <Nav pills>
                        ...
                        <Switch>
                            ...
                            {/* <Route path="/(validator|validators)/:address/delegations" render={() => <ValidatorDelegations address={this.props.validator.address} tokens={this.props.validator.tokens} shares={this.props.validator.delegator_shares} />} /> */}
                            {/* <Route path="/(validator|validators)/:address/transactions" render={() => <ValidatorTransactions validator={this.props.validator.address} delegator={this.props.validator.delegator_address} limit={100}/>} /> */}
                      </Switch>
